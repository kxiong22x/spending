...
import { BUILTIN_CATEGORIES_LOWER_SET, MONTH_REGEX, MAX_NAME_LENGTH } from '../constants';
...
router.post('/', async (req, res, next: NextFunction) => {
  ...
  const { month, is_recurring } = req.body as { month?: string; is_recurring?: boolean };
  ...
  try {
    ...
    if ((err as Error).message.includes('UNIQUE')) return res.status(409).json({ error: `Category "${name}" already exists for this month` });
    return next(err);
  }
