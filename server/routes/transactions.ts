...
async function copyRecurringCategories(userId: number, targetMonth: string): Promise<void> {
...
router.post('/upload', async (req, res) => {
  const { month, cardRows } = req.body as { month?: string; cardRows?: unknown };
  ...
  const totalRows = (cardRows as Array<{ rows?: unknown[] }>).reduce(
    (sum, cr) => sum + (Array.isArray(cr.rows) ? cr.rows.length : 0),
    0
  );
  if (totalRows > 5000) {
  ...
  const cardIds = (cardRows as Array<{ cardId: number }>).map(cr => cr.cardId);
  ...
  for (const { cardId, rows } of cardRows as Array<{ cardId: number; rows: unknown[] }>) {
    ...
    for (const row of rows as Array<Record<string, unknown>>) {
      if (!isValidDate(row.date) || typeof row.amount !== 'number' || !isFinite(row.amount)) {
        ...
        skipped++;
      } else if ((row.description as string).length > MAX_DESCRIPTION_LENGTH) {
        skipped++;
      } else if (!(row.date as string).startsWith(month)) {
        skipped++;
      } else if (AUTOPAY_PATTERNS.some(p => (row.description as string).toLowerCase().includes(p))) {
        skipped++;
      } else {
        validRows.push({ ...(row as Omit<CsvRow, 'cardId'>), cardId });
      }
    }
  }
  ...
  const validCustomCategories = new Set(customCatRows.rows.map(r => r.name as string));
  ...
  const learnedRules = new Map(rulesRows.rows.map(r => [r.pattern as string, r.category as string]));

  // Classify all valid rows, checking learned rules before calling Gemini
  let categories: string[], matchedPatterns: string[];
  try {
...
router.post('/', async (req, res) => {
  const { date, description, category, amount, card_id } = req.body as {
    date?: unknown;
    description?: unknown;
    category?: unknown;
    amount?: unknown;
